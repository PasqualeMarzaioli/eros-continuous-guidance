...
plotValues = differences;
plotValues(:, 1) = 1000 * differences(:, 1);
plotValues(:, 2) = 1e6 * differences(:, 2);
days = time / 86400;
...
labels = {'\Delta a / m', '\Delta e / 10^{-6}', '\Delta i / deg', ...
    '\Delta \Omega / deg', '\Delta u / deg'};
for index = 1:5
    nexttile(layout);
    plot(days, plotValues(:, index), 'k-', 'LineWidth', 1.1);
